...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-approval-consumer-' . bin2hex(random_bytes(6));
...
$archive = $distDirectory . '/kumwe-approval.zip';
if (!is_file($archive)) {
...
if (
    ...
    || !is_array($manifest)
    || !is_string($release)
    || $release !== 'unreleased' && preg_match('/^[0-9]+\.[0-9]+\.[0-9]+$/D', $release) !== 1
) {
...
$release = $release === 'unreleased' ? 'dev-candidate' : $release;
...
$consumerMetadata = [
    ...
    'license' => 'proprietary',
    'require' => ['kumwe/approval' => $release, 'laminas/laminas-servicemanager' => '^4.0', ...array_filter($metadata['require'], static fn (string $constraint, string $name): bool => str_starts_with($name, 'kumwe/'), ARRAY_FILTER_USE_BOTH)],
    'repositories' => [
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\Approval\\Tests\\')
    ) {
